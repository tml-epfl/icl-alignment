"""
The code is modified from the URIAL repository.
https://github.com/Re-Align/URIAL/blob/main/src/unified_infer.py
"""
...
import json
import os  
from unified_utils_v2 import load_eval_data, save_outputs
from unified_utils_v2 import openai_chat_request, openai_base_chat_request, retry_handler
...
def parse_args():
    ...
    parser.add_argument('--api_org', default="", type=str)
...
if __name__ == "__main__":
    ...
    if args.engine == "vllm":
        ...
        llm = LLM(model=args.model_name, tokenizer=args.tokenizer_name, dtype=args.dtype, tokenizer_mode=args.tokenizer_mode, trust_remote_code=True)    
    ...
    if args.engine == "vllm": 
        ...
        def api(**kwargs):
            if "base" in args.model_name.lower():
                result = openai_base_chat_request(**kwargs)
                return result
